<script lang="ts">
	...
	import { deletePage, movePage } from '#lib/controllers/content.remote.ts';
	import type { PageRow } from '#lib/controllers/pages.controller.ts';
	import { LOCALES, LOCALE_INFO } from '#lib/site.ts';
	...
	async function move(row: PageRow, direction: 'up' | 'down') {
		await movePage({ slug: row.slug, direction });
		await invalidateAll();
	}

	async function remove() {
		if (!confirming) return;
		await deletePage(confirming.slug);
		dialog?.close();
...
<svelte:head>
	<title>Pages · Knowledge Base Editor</title>
</svelte:head>
	...
	<div class="flex flex-wrap items-end gap-4">
		<div class="flex grow flex-col gap-1">
			...
			<p class="text-[0.9375rem] text-ink-3">
				Every page on the knowledge base. Choose one to edit, or start a new one.
			</p>
		</div>
		<a href={resolve('/pages/new')} class="btn min-h-11 gap-2 btn-primary px-5">
	...
	{#each groups as group (group.id)}
		<section class="flex flex-col gap-2">
			<div class="flex items-center gap-3">
				...
				<a
					href={resolve('/pages/new') + `?topic=${group.id}`}
			...
			<ul class="overflow-hidden rounded-lg border border-line bg-raised">
				{#each group.pages as row, i (row.slug)}
					...
					>
						<a
							href={resolve('/pages/[slug]', { slug: row.slug })}
						...
						<div class="flex items-center gap-1.5">
							{#each LOCALES as locale (locale)}
								{@const status = row.translations[locale]}
								<span
									class={[
										'chip',
										status.state === 'complete' && 'border-brand-line bg-brand-soft text-[#166534]',
										status.state === 'partial' && 'border-warn-line bg-warn-soft text-warn-ink',
										status.state === 'none' &&
											'border-neutral-line bg-neutral-soft text-neutral-ink'
									]}
								>
									{LOCALE_INFO[locale].label}
									{#if status.state === 'partial'}
										: {status.missing} to translate
...
>
	<div class="flex flex-col gap-4 p-6">
		<h2 class="text-xl font-semibold">Delete “{confirming?.title}”?</h2>
		<p class="text-[0.9375rem] leading-6 text-ink-3">
			The page and its pictures are removed from this computer. Links to it from other pages stop
			working. The website keeps the page until you publish.
		</p>
